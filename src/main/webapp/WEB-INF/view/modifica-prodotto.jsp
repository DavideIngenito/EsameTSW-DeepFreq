...
<head>
  ...
  <meta charset="UTF-8">
  <title>DeepFreq | Modifica Prodotto</title>
  ...
  <style>
    ...
    .admin-form button { background: #3498db; color: #fff; padding: 15px; border: none; width: 100%; font-weight: bold; cursor: pointer; margin-top: 25px; text-transform: uppercase; }
...
<main class="admin-container">
  <h2><i class="fas fa-edit" aria-hidden="true"></i> Modifica Prodotto</h2>

  <c:if test="${not empty errore}">
    <div class="error-msg" role="alert">${errore}</div>
  </c:if>

  <form action="${pageContext.request.contextPath}/modificaProdotto" method="POST" class="admin-form">
    ...
    <input type="text" id="artista" name="artista" value="${prodotto.artista}" required>

    <div style="display: flex; gap: 20px;">
      <div style="flex: 1;">
        <label for="categoria">Categoria:</label>
        <select id="categoria" name="categoria">
          <option value="Vinile" <c:if test="${prodotto.categoria == 'Vinile'}">selected</c:if>>Vinile</option>
          <option value="Audio Gear" <c:if test="${prodotto.categoria == 'Audio Gear'}">selected</c:if>>Audio Gear</option>
        </select>
      </div>
      <div style="flex: 1;">
        <label for="genere">Genere / Tipo:</label>
        <input type="text" id="genere" name="genere" value="${prodotto.genere}" required>
      </div>
    </div>
    ...
    <div style="display: flex; gap: 20px;">
      <div style="flex: 1;">
        ...
        <input type="number" id="anno_uscita" name="anno_uscita" value="${prodotto.annoUscita}" min="1900" max="2030" step="1" required>
      </div>
      <div style="flex: 1;">
        <label for="prezzo">Prezzo (€):</label>
        <input type="number" id="prezzo" name="prezzo" step="0.01" min="0.01" value="${prodotto.prezzo}" required aria-label="Prezzo del prodotto in euro">
      </div>
      <div style="flex: 1;">
        <label for="quantita">Quantità Magazzino:</label>
        <input type="number" id="quantita" name="quantita" value="${prodotto.quantitaMagazzino}" min="0" step="1" required aria-label="Quantità disponibile">
      </div>
    </div>

    <label for="immagine">Nome Immagine:</label>
    <input type="text" id="immagine" name="immagine" value="${prodotto.urlImmagine}" required>

    <label for="tracklist">Tracklist (Titolo - MM:SS):</label>
    <textarea id="tracklist" name="tracklist" rows="6">${tracklist}</textarea>

    <label for="descrizione">Descrizione:</label>
    <textarea id="descrizione" name="descrizione" rows="4" required>${prodotto.descrizione}</textarea>
